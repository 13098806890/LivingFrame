...
`1.0/` is a full baseline captured from the current App Store Connect draft on 2026-09-17. It includes every supported localization, current price rows, and availability snapshots for the weekly, monthly, annual, and lifetime products. Run `fastlane ios sync_purchase_pricing version:1.0` after changing prices or territories to refresh these read-only snapshots.
